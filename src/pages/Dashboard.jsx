...
export default function Dashboard() {
    ...
    return (
      <div className="p-4 sm:p-6 bg-gray-50 min-h-screen">
        <div className="bg-white rounded-xl shadow p-6 sm:p-8 text-center max-w-md mx-auto mt-8 sm:mt-10">
          <h2 className="text-lg sm:text-xl font-bold text-red-600 mb-4">Data Not Available</h2>
  ...
  const subjects = Object.keys(data || {}).map(subjectKey => ({
    id: subjectKey,
    name: subjectKey.split('-').map(word => word.charAt(0).toUpperCase() + word.slice(1)).join(' '),
    chapters: data[subjectKey]?.chapters || [],
    // Use student's performance data or default to random progress
    progress: studentData.performance?.[subjectKey] || Math.floor(Math.random() * 100)
  }));

  const totalSubjects = subjects.length;
  const totalChapters = subjects.reduce((total, subject) => 
    total + (subject.chapters?.length || 0), 0
  );
  
  // Calculate average progress based on student's performance
  const avgProgress = totalSubjects > 0 
    ? Math.round(
        subjects.reduce((total, subject) => total + (subject.progress || 0), 0) / totalSubjects
      )
    : 0;

  // Get student's recent activity (would come from backend in real app)
  const recentActivity = [
    { subjectId: "mathematics", chapterId: 1, title: "Rational Numbers", time: "Today", duration: "45 mins" },
    { subjectId: "science", chapterId: 1, title: "Crop Production", time: "Yesterday", duration: "1 hour" },
    { subjectId: "english", chapterId: 1, title: "The Best Christmas Present", time: "2 days ago", duration: "30 mins" }
  ];
  ...
  return (
    <div className="flex bg-gray-50 mt-7">
      ...
      <div className="flex-1 md:ml-64 p-3 sm:p-4 sm:pt-20 md:pt-6 mt-11.25">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 sm:gap-4">
            <div className="flex-1 min-w-0">
              <h1 className="text-lg sm:text-xl md:text-2xl font-bold text-gray-800 flex items-center gap-2">
                ...
                <span className="truncate">Welcome back, {studentData.name}!</span>
              </h1>
            ...
            <div className="flex items-center gap-2 sm:gap-3 md:gap-4 bg-white p-2 sm:p-3 rounded-lg shadow-sm w-full sm:w-auto">
              <div className="text-center flex-1 sm:flex-none">
                ...
                <p className="font-bold text-blue-900 text-sm sm:text-base">{studentData.totalStudyHours}h</p>
              </div>
              <div className="h-6 w-px bg-gray-300 hidden sm:block"></div>
              <div className="text-center flex-1 sm:flex-none">
                <p className="text-xs text-gray-500">Chapters Done</p>
                <p className="font-bold text-green-600 text-sm sm:text-base">{studentData.completedChapters}</p>
              </div>
            </div>
          </div>
        </div>
        ...
        {/* Motivation Banner */}
        <div className="bg-linear-to-r from-blue-900 to-indigo-700 text-white p-3 sm:p-4 md:p-5 rounded-xl mb-4 sm:mb-6">
          ...
          <div className="mt-2 sm:mt-3 md:mt-4">
            ...
            <div className="flex justify-between text-xs mt-1">
              <span>0%</span>
              <span className="text-xs sm:text-sm">Current: {avgProgress}%</span>
              <span>100%</span>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-3 gap-2 sm:gap-3 md:gap-4 mb-4 sm:mb-6">
          <StatCard
            icon={<BookOpen className="text-blue-900 w-4 h-4 sm:w-5 sm:h-5 md:w-6 md:h-6" />}
            title="Enrolled Subjects"
            value={totalSubjects}
            description="You are studying"
          />
          <StatCard
            icon={<CheckCircle className="text-green-600 w-4 h-4 sm:w-5 sm:h-5 md:w-6 md:h-6" />}
            title="Available Chapters"
            value={totalChapters}
            description="Total chapters to learn"
          />
          <StatCard
            icon={<BarChart3 className="text-purple-600 w-4 h-4 sm:w-5 sm:h-5 md:w-6 md:h-6" />}
            title="Average Score"
            value={`${avgProgress}%`}
        ...
        <div className="mb-4 sm:mb-6">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-4 gap-2 sm:gap-3 md:gap-4">
            <QuickAction
              icon={<PlayCircle className="w-4 h-4 sm:w-5 sm:h-5 md:w-6 md:h-6" />}
              title="Continue Learning"
              link={`/tutorials/class/${classId}`}
              color="blue"
            />
            <QuickAction
              icon={<ClipboardList className="w-4 h-4 sm:w-5 sm:h-5 md:w-6 md:h-6" />}
              title="Take Quiz"
            ...
            />
            <QuickAction
              icon={<BarChart3 className="w-4 h-4 sm:w-5 sm:h-5 md:w-6 md:h-6" />}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2 sm:gap-3 md:gap-4">
            {subjects.map((subject) => (
              <Link
                ...
                className="group"
              >
                <div className="bg-white p-3 sm:p-4 rounded-xl shadow hover:shadow-lg transition-all hover:scale-[1.02]">
                  <div className="flex justify-between items-start mb-2 sm:mb-3">
                    ...
                    <span className={`text-xs sm:text-sm font-medium px-1.5 sm:px-2 py-0.5 sm:py-1 rounded ${
                      ...
                      subject.progress >= 60 ? "bg-blue-100 text-blue-800" :
          ...
          <div className="bg-white rounded-xl shadow overflow-hidden">
            ...
            {recentActivity.length === 0 && (
              <div className="p-4 sm:p-6 md:p-8 text-center text-gray-500">
                ...
                <p className="text-xs sm:text-sm mt-1">Start learning to see your activity here!</p>
              </div>
            )}
          </div>
        </div>
        ...
        {/* Daily Learning Goal */}
        <div className="mt-4 sm:mt-6 p-3 sm:p-4 bg-linear-to-r from-green-50 to-blue-50 rounded-xl border border-green-200">
          ...
          <div className="mt-2 sm:mt-3 flex items-center gap-2">
            <div className="flex-1 bg-gray-200 rounded-full h-1.5 sm:h-2">
              <div className="bg-green-500 h-1.5 sm:h-2 rounded-full" style={{ width: '40%' }}></div>
            </div>
            <span className="text-xs text-gray-600 whitespace-nowrap">40% complete</span>
          </div>
        </div>
      </div>
    </div>
...
function QuickAction({ icon, title, link, color = "blue" }) {
  const colorClasses = {
    ...
    purple: "bg-purple-100 text-purple-900 hover:bg-purple-900 hover:text-white",
  ...
  return (
    ...
    >
      <div className={`p-1.5 sm:p-2 md:p-3 rounded-full transition ${colorClasses[color]}`}>
        {icon}
      </div>
